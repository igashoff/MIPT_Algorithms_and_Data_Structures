...
//  Функция, окторая считает ширину дерева tree.
int countWidth( CNode* tree );
...
int main()
{
    ...
    // Считаем ширину декартова дерева.
    int treapWidth = countWidth( treap );
    ...
    int binaryTreeWidth = countWidth( binaryTree );
    
    // Выводим разность высот.
    cout << treapWidth - binaryTreeWidth;
...
//  Пусть мы находимся на i-м уровне и текущая максимальная ширина равна maxWidth. Тогда ptrs будет содержать ненулевые указатели на все узлы,
//  которые находятся на уровне i + 1. Будем так спускаться, пока вектор ptrs не станет пустым.
//  По ходу спуска сравниваем текущую maxWidth с количеством элементов в ptrs - это и есть ширина уровня i + 1.
int countWidth( CNode* tree )
{
    if( tree == nullptr ) {
        return 0;
    ...
    while( !ptrs.empty() ) {
        maxWidth = max( maxWidth, static_cast<int>( ptrs.size() ) );
        vector<CNode*> temp( 0 );
        for( CNode* ptr: ptrs ) {
            if( ptr->left != nullptr ) {
